...
public enum StatusVeiculoEnum {
	 ...
	 */
	private StatusVeiculoEnum(Integer status, String descricao) {
		this.status = status;
		this.descricao = descricao;
	}
	
	public static StatusVeiculoEnum valueOfStatus(Integer status) {
		Optional<StatusVeiculoEnum> optional = Arrays.asList(values()).stream()
				.filter(StatusVeiculoEnum -> StatusVeiculoEnum.getStatus().equals(status)).findFirst();
